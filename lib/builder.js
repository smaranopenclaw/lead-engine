...
- No canonical → <link rel="canonical" href="#">
...
export async function buildSiteHTML(params) {
  ...
  try {
    ...
    // Run claude headlessly via spawnSync — avoids shell escaping issues entirely
    ...
    if (result.status !== 0) throw new Error(`claude exited ${result.status}: ${result.stderr?.slice(0, 500)}`)
